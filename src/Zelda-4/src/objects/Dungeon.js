...
export default class Dungeon {
	...
	constructor(player) {
		...
		canvas.addEventListener(EventName.ShiftUp, () => this.beginShifting(new Vector(0, -CANVAS_HEIGHT)));
		canvas.addEventListener(EventName.ShiftDown, () => this.beginShifting(new Vector(0, CANVAS_HEIGHT)));
		canvas.addEventListener(EventName.ShiftLeft, () => this.beginShifting(new Vector(-CANVAS_WIDTH, 0)));
		canvas.addEventListener(EventName.ShiftRight, () => this.beginShifting(new Vector(CANVAS_WIDTH, 0)));
	}
	...
	beginShifting(nextRoomPosition) {
		...
		const newPlayerPosition = this.prepareRoomAndPlayerForShift(nextRoomPosition);
	...
	prepareRoomAndPlayerForShift(nextRoomPosition) {
		...
		this.nextRoom.adjacentOffset.set(nextRoomPosition.x, nextRoomPosition.y);

		return this.getNewPlayerPosition(nextRoomPosition);
	}

	getNewPlayerPosition(nextRoomPosition) {
		let newPlayerPosition = new Vector(this.player.position.x, this.player.position.y);

		if (nextRoomPosition.x > 0) {
			newPlayerPosition.x = CANVAS_WIDTH + (Room.RENDER_OFFSET_X + Tile.TILE_SIZE);
		}
		else if (nextRoomPosition.x < 0) {
			newPlayerPosition.x = -CANVAS_WIDTH + (Room.RENDER_OFFSET_X + (Room.WIDTH * Tile.TILE_SIZE) - Tile.TILE_SIZE - this.player.dimensions.x);
		}
		else if (nextRoomPosition.y > 0) {
			newPlayerPosition.y = CANVAS_HEIGHT + (Room.RENDER_OFFSET_Y + this.player.dimensions.y / 2);
		}
		else {
			newPlayerPosition.y = -CANVAS_HEIGHT + Room.RENDER_OFFSET_Y + (Room.HEIGHT * Tile.TILE_SIZE) - Tile.TILE_SIZE - this.player.dimensions.y;
		}
	...
	tweenRoomAndPlayer(nextRoomPosition, newPlayerPosition) {
		timer.tween(this.camera, ['x', 'y'], [nextRoomPosition.x, nextRoomPosition.y], 1, () => {
			timer.tween(this.player.position, ['x', 'y'], [newPlayerPosition.x, newPlayerPosition.y], 1, () => {
	...
	resetPlayer(newRoomPosition) {
		if (newRoomPosition.x < 0) {
			this.player.position.x = Room.RENDER_OFFSET_X + (Room.WIDTH * Tile.TILE_SIZE) - Tile.TILE_SIZE - this.player.dimensions.x;
			...
			this.player.position.y = Room.RENDER_OFFSET_Y + (Room.HEIGHT * Tile.TILE_SIZE) - Tile.TILE_SIZE - this.player.dimensions.y;
			this.player.direction = Direction.Up;
		}
		else {
			this.player.position.y = Room.RENDER_OFFSET_Y + this.player.dimensions.y / 2;
